 ...
 * - back button to leave guest view
 * - only name is required in guest form
  ...
  const component = {
    ...
    Instance: function () {
      ...
      this.start = async function () {
         ...
         * @type {string}
         */
        const received_guest_data = new URL( window.location.href ).searchParams.get( 'contact' );

        /**
         * true if this device already has stored guest data
         * @type {boolean}
         */
        const has_contacts = await this.store.count();

        if ( received_guest_data )    // has received guest data?
          renderReceivedGuestData();  // => show it
        else if ( has_contacts )      // has already saved guests data?
        ...
        function guestView() {
          ...
          function renderGuestForm( initial_values ) {
            ...
            $.render( $.html( self.html.guestForm, onSubmit, onCancel, onDelete, initial_values, 'Zurück zur Liste', onBack, has_contacts ), self.element );
        ...
        async function renderReceivedGuestData() {
          ...
          async function onSave() {
            ...
            window.location = location.origin + location.pathname;  // remove encoded guest data from web URL (restarts the app)
          }
